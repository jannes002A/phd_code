...
%         end
%         
%         Bx = Basisfunc(Xtemp,count,Xm(1:count),sigma,0);
...
plot(x,-dpoth(x)'+ [ddpoth(xh)+2,zeros(1,50)]',x,-dpoth(x)','LineWidth',3);
legend('Pot+Bias','Pot')

figure(2)
Bx = Basisfunc(x,count,Xm(1:count),sigma,1); 
dVbias = tau*omega*sum(Bx,2);
plot(x,poth(x)'+ [ddpoth(xh)+2,zeros(1,50)]',x,poth(x),'LineWidth',3);
legend('Pot+Bias','Pot')
