...
rng(12,'twister')                 % restore the previous settings
...
% eine Trajektorie um das Bias Potential zu berechnen. 
 ...
 %while( (Xtemp < -0.9 && Xtemp > -1.1)==0 )
        ...
        Bx = Basisfunc2(Xtemp,count,Xm(1:count),sigma(1:count),0);
...
%         end
        ...
        Bx = Basisfunc2(Xtemp(j),count,Xm(1:count),sigma,0);
        dVbias = tau*omega*ones(1,count) * Bx';
...
fprintf('E[time]: %2.8f \n',mean(time))
        ...
        x=linspace(-1.5,1.5);
        Bx = Basisfunc2(x,count,Xm(1:count),sigma,1);
        dVbias = tau*omega*ones(1,count) * Bx';

        figure(1)
        plot(x,dVbias,'r',x,-dpot,'b',x,dVbias-dpot,'g','LineWidth',3); 
        legend('Bias','-dPot','dVbias-dPot')
        
        x=linspace(-1.5,1.5);
        Bx = PotBasisfunc(x,count,Xm(1:count),sigma,1);
        Vbias = tau*omega*ones(1,count) * Bx';

        figure(2)
        plot(x,Vbias,'r',x,pot,'b',x,-Vbias+pot,'g','LineWidth',3); 
        legend('Bias','Pot')
