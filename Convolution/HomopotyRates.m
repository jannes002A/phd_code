...
for var=1:nvar
     ...
     % Schleife simuliert S Porzesse gleichzeitig
      ...
      for l=1:max(pos)
        num(i,l,var)= length(find(pos>=l,1000));
      end
     end
end


rate=zeros(length(t),nvar);


for i=1:length(t)
    for k=1:nvar
     pos_help=find(num(i,:,k)> 0,1,'last');
     time=(1:pos_help);
     prob=log(num(i,1:pos_help,k));
...
for i=1:length(t)
    mrate(i) = mean(rate(i,:));
    sigma(i)= sum((rate(i,:)-mrate(i)).^2);
    cc(length(t)+1-i) = 1.96*sqrt(sigma(i)/nvar);
end

numh=zeros(length(t),N);
for i=1:nvar
        numh = numh + num(:,:,i);
end
numh=numh./nvar;
...
%  set(l1,'Interpreter','Latex');
 ...
 xl=xlabel('time');
 yl=ylabel('Number of Processes in Well');
 hold off
 ...
 %cc=zeros(1,length(t));
 % Approximation der rate durch eine Grade
 for l=1:length(t)
     pos_help=find(num(l,:)> 0,1,'last');
     time=(1:pos_help);
     prob=log(num(l,1:pos_help));
     p=polyfit(time,prob,1);
     rate(l)=p(1);
     %sigma(l)= sqrt(((sum(prob-(p(1)*time+p(2)))^2))/N);
     %cc=1.96*(sigma(l)/sqrt(N));
 end
...
%  plot(t,rate)
 ...
 pos_help=find(-rate>1E-20,1,'last');
...
%  semilogy(t,yfit4)
 ...
 % extrapolation ohne ausreißer
 
 %pos_help=find(t>0.025,1,'last');
 helper= (-rate>1E-15);
 pos_help=find(helper>0,1,'last');
 rate_help=log(-rate(1:pos_help));
 ...
 xl=xlabel('position space');
 yl=ylabel('potential energy');
 hold off
 ylim([-0.5 17 ])
 ...
 set(yl,'Interpreter','Latex');
  ...
  t=[0.08,0.07,0.06,0.05,0.04,0.03,0.02];
 homopoth =@(y,h) poth(y)+96.*h.^2+(4-88.*y+96.*y.^2).*h;
 figure(12)
 ...
 hold off
 ylim([0 8 ])
 l=legend('$\lambda = 0.08 $','$\lambda = 0.07 $','$\lambda = 0.06 $','$\lambda = 0.05 $','$\lambda = 0.04 $','$\lambda = 0.03 $','$\lambda = 0.02 $','Location','northwest');
 set(l,'Interpreter','Latex');
 set(xl,'Interpreter','Latex');
 set(yl,'Interpreter','Latex');
 ...
 xl=xlabel('position space');
 yl=ylabel('Boltzman distribution');
 hold off
l=legend('$\lambda = 0 $','$\lambda = 0.04 $','$\lambda = 0.08 $','$\lambda = 0.12 $','$\lambda = 0.16 $','Location','northwest');
set(l,'Interpreter','Latex');
 set(xl,'Interpreter','Latex');
...
hold on
plot(t(1:2:pos_help),log(-rate(1:2:pos_help)),'ko','LineWidth',4);
%errorbar(t(1:5:pos_help),lrate(1:5:pos_help),sigma(1:5:pos_help),'r.')
hold off
l=xlabel('parameter $\lambda$');
l1=ylabel('log(Rates)');
legend('Extrapolated Rates','Real Rates','Location','southeast')
...
figure(13)
errorbar(t(1:2:pos_help),(-rate(1:2:pos_help)),cc(1:2:pos_help),'r.')