...
for k=1:num_iter
    ...
    [Ecost,Ehit,Egrad,VHit,VCost,VObjf,VGrad] = mean_controlled_hittingtimeSG( x0,BETA,a,b,POT,coeffs(k,:),leftlim,rightlim,n);
    grad_tildeI = Egrad;
    VarHit(k)=VHit;
    VarCost(k)= VCost;
    VarObjf(k) = VObjf;
...
[Ecost,Ehit,Egrad,VHit,VCost,VObjf,VGrad] = mean_controlled_hittingtimeSG( x0,BETA,a,b,POT,coeffs(num_iter+1,:),leftlim,rightlim,n);
grad_tildeI = Egrad;
...
h2=figure(2);
plot(1:1:length(opt_coeffs),err_coeffs(11,:));
...
h3=figure(3);
plot(1:1:length(opt_coeffs),abserr_coeffs(11,:));
xlabel('j-th coefficient')
ylabel('|(c^i-c)_j|')
