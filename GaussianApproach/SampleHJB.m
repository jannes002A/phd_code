...
for opt = 1:opt_steps
   ...
   c_pred = -K_pred*c*dt - K_pred*(data_nonbias); 
...
plot(x_pred,   c_old(end,:)' -gradV(x_pred)'  )
...
plot(x_pred,per_pot(2:end)+6.2); hold off
title('Perturbed Potential')

figure(8)
plot(x_pred,V(x_pred)); hold on
plot(x_pred,V(x_pred)+control(2:end)+4); hold off
