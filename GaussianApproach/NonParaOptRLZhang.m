    ...
    for i = 1:ntrjs
        ...
        for j = 2:nsteps
            ...
            Id = Id - bias.^2 / sigma^2 *dt;
             ...
             if  x < -0.9 && x > -1.1 
                 time(i) = j;
                 pathfunc(i) = (j*dt + 1/4*Bias(i,:)*Bias(i,:)'*dt)*exp(Is+0.5*Id); %weighted path functional 
                 Girsnaov(i) = exp(Is+0.5*Id);
                 X(i,j:end)=x;
                 X_nonbias(i,j:end) = x-bias*dt;
                 break;
             else
                 pathfunc(i) = (j*dt + 1/4*Bias(i,:)*Bias(i,:)'*dt)*exp(Is+0.5*Id);
             end
        end
