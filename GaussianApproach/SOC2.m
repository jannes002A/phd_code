...
for opt = 1:opt_steps
    ...
    for i = 1:ntrjs
        ...
        while (x<0) 
            ...
            if opt==1
                ...
                K_pred = pathweight./((ntrjs)).* sk/sqrt(2*pi)*l.^2.*exp(-0.5*(data'-x).^2/l.^2);
                bias =  -K_pred'*dataB'+K_pred'*c*dt ;  
                x = x + (-gradV(x)-bias)*dt + eta * sigma*sdt;
            end
            X(j) = x;
            B(j) = eta*sdt;
    ...
    A = -1/(2*beta)*K*dt + (1.)*speye(length(data),length(data)); 
    b = 1/sqrt(2*beta)*K*B';
   ...
   for i=1:length(x_pred)
       K_pred(i,:) = (pathweight/(ntrjs)).* sk.*exp(-0.5*(data'-x_pred(i)).^2/l.^2);
   end
   c_pred = - 1/sqrt(2*beta)*K_pred*B'+ 1/(2*beta)*K_pred*c*dt ;  
...
figure(8)
plot(x_pred,V(x_pred),x_pred,V(x_pred)+control(2:end)+1.5,'LineWidth',3); 
legend('Potential','Perturbed Potential')
