...
for opt = 1:opt_steps
    ...
    for i = 1:ntrjs
        ...
        for k=1:length(X)
            Ki = Ki+sk.*exp(-0.5*(X(k)-points).^2/l.^2)*X(k);
            khelp= sk.*exp(-0.5*(X(k)-points).^2/l.^2);
            Kii = Kii + khelp'*khelp *dt;
        end
        
        bhelp(i,:)= pathfunc(i)*Ki;
        A = A + pathfunc(i)*Kii;
    ...
    K_pred=zeros(length(x_pred),npoints);
    for k=1:npoints
        K_pred(:,k) = sk.*exp(-0.5*(x_pred-points(k)).^2/l.^2);
    end
    
    figure(opt)
    plot(x_pred,K_pred*par);
...
figure(6)
plot(x_pred,  -gradV(x_pred),x_pred, -gradV(x_pred)- par'*K_pred','LineWidth',3 ); 
...
figure(8)
plot(x_pred,V(x_pred),x_pred,V(x_pred)+control(2:end)+0.3,'LineWidth',3); 
legend('Potential','Perturbed Potential')
